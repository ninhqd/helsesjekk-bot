...
async function Page(): Promise<ReactElement> {
  ...
  const sortedTeams = R.pipe(
    teams,
    R.sortBy(
      [(team) => team.active, "desc"],
      [(team) => team.Asked.length, "desc"]
    )
  );
          ...
          <div key={team.id} className="bg-bg-subtle p-4 pt-2 rounded">
            <div className="flex justify-between mb-2">
              ...
              >
                ...
                {team.assosiatedGroup != null && (
                  <HandShakeHeartIcon title="Har satt opp dashboard" />
                )}
              </Heading>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <TeamDetail
                text="Spør"
                detail={`${dayIndexToDay(team.postDay)}, kl. ${
                  team.postHour
                }:00`}
              />
              <TeamDetail
                text="Viser"
                detail={`${dayIndexToDay(team.revealDay)}, kl. ${
                  team.revealHour
                }:00`}
              />
              ...
              <TeamDetail
                text="Uten svar"
                detail={`${
                  team.Asked.filter((it) => it._count.answers === 0).length
                }`}
              />
              <TeamDetail
                text="Spørsmål"
                detail={`${questionsFromJsonb(team.questions).length}`}
              />
              <TeamDetail
                text="Egne spørsmål"
                detail={`${
                  questionsFromJsonb(team.questions).filter((it) => it.custom)
                    .length
                }`}
              />
            </div>
...
function getAdminGroupId(): string {
  ...
  return "7058393f-e989-463d-9a4d-b01e45cb4e35";
}
