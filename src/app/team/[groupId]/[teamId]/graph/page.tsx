...
import {
  getTeamScorePerQuestion,
  getTeamScoreTimeline,
} from "../../../../../db/score";
...
async function OverallGraph({
    ...
    return (
      <div className="mb-4">
        ...
        <div className="w-full aspect-video relative flex items-center justify-center flex-col border rounded border-border-subtle">
          ...
          <BodyLong>
            Det er ingen data å vise for teamet ditt. Dette kan skyldes at
            teamet ditt ikke har svart på noen spørsmål enda.
          </BodyLong>
        </div>
      </div>
    );
  ...
  return (
    <div>
      ...
      <Detail>
        {scoreTimeline.length} målinger siden Uke{" "}
...
async function PerQuestionGraph({
  teamId,
}: {
  teamId: string;
}): Promise<ReactElement> {
  ...
  return (
    <div>
      ...
      <Detail>
        {scoredQuestions.length} målinger siden Uke{" "}
        {getWeekNumber(earliest.timestamp)}, {earliest.timestamp.getFullYear()}
      </Detail>
      <div className="mt-4">
        <ScorePerQuestion
