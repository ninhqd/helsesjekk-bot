...
async function PreviousAskedView({
  teamId,
}: {
  teamId: string;
    ...
    return (
      <div className="mb-4">
        <div className="">
          ...
          <BodyLong>
            Det er ingen tidligere spørringer å vise for teamet ditt. Dette kan
            skyldes at teamet ditt ikke har svart på noen spørsmål enda, eller
            at ingen av spørringene hadde 3 eller flere svar.
          </BodyLong>
        </div>
      </div>
    );
  ...
  const withPrevious = R.pipe(
    scoredAsks,
    R.reverse(),
    (it) => R.zip.strict(it, [null, ...it]),
    R.reverse()
  );
  ...
  return (
    <div>
      <Detail>
        {scoredAsks.length} målinger siden uke{" "}
        {getWeekNumber(earliest.timestamp)}, {earliest.timestamp.getFullYear()}
      </Detail>
      <div className="mt-4 flex flex-wrap gap-4">
        {withPrevious.map(
          ([currentAsk, previousAsk]: [ScoredAsk, ScoredAsk | null]) => (
            <ScoredAskView
...
function ScoredAskView({
  ...
  const totalDiff = previousAsk
    ? ask.totalScore - previousAsk.totalScore
    : null;
  return (
        ...
        <div key={type}>
          ...
          {questions.map((question) => {
            const previousQuestion =
              previousAsk?.scoredQuestions.find(
                (it) => it.id === question.id
              ) ?? null;
            const diff = previousQuestion
              ? question.score - previousQuestion.score
              : null;
