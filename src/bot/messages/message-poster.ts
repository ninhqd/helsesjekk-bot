...
export async function postToTeam(
  ...
  const text = `Hvordan har ${team.name} det?`;
  const blocks = [
  ...
  if (!message.ok || message.ts == null) {
    console.error(
      `Unable to post message for team ${team.name}, error: ${
...
export async function remindTeam(
  team: Team,
  client: App["client"]
): Promise<boolean> {
...
export async function updateResponseCount(
  team: Team,
  client: App["client"]
): Promise<boolean> {
  const asked = await getActiveAsk(team.id);
  ...
  if (!message.ok) {
    console.error(
      `Unable to update message for team ${team.name}, error: ${
        message.error ?? "Unknown error"
      }`
    );
    return false;
...
export async function revealTeam(
  team: Team,
  client: App["client"]
): Promise<boolean> {
  const asked = await getActiveAsk(team.id);
