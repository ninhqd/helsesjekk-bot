...
import {
  ...
  getIdValueFromAnswer,
  HealthcheckModalActions,
} from "./healthcheck-modal-builder";
...
export function configureHealthCheckEventsHandler(app: App): void {
  ...
  app.action(
    MessageActions.FillButtonClicked,
    async ({ ack, action, body, client }) => {
      ...
      if (team == null || asked == null) {
        ...
        await client.chat.postEphemeral({
          channel: channelId,
          text: ":huh: Det ser ut som du svarer på et spørsmål som aldri er spurt. :scream_cat: Kan du ta kontakt i <#C06K8N4MZEG>? :smile:",
          user: userId,
        });
        return;
      }
  ...
  app.view(
    HealthcheckModalActions.modalSubmit,
    async ({ ack, view, body, client }) => {
      ...
      if (team == null || asked == null) {
        ...
        return ack({
          ...
          errors: {
            "feedback-block":
              "Det ser ut som du svarer på et spørsmål som aldri er spurt. Kan du ta kontakt i #sam-helsesjekk-bot?",
          },
        });
      }
