...
export function createSettingsModal(team: Team, isAdding = false): ModalView {
  ...
  return {
    ...
    blocks: [
      ...
      textSection(
        "For å endre på frekvensen på helsesjekken og andre innstillinger, besøk https://helsesjekk-bot.bks-prod.politiet.no/"
      ),
      plainHeader("Frekvens"),
      textSection(
        team.frequency === 1 ? "Hver uke" : `Hver ${team.frequency}. uke`
      ),
      textSection(
        `Dersom botten er aktiv, er neste helsesjekk ${dayIndexToDay(
          team.postDay
        )} kl. ${team.postHour}:00 ${nextOccurenceText(postDate)}`
      ),
...
function customQuestionsSection(
  ...
  const customQuestions = questionsFromJsonb(team.questions).filter(
    (it) => it.custom
  );
        ...
        (it) =>
          ({
            ...
            accessory: {
              ...
              confirm: {
                text: text(
                  `Er du sikker på at du vil slette spørsmålet "${it.question}"`
                ),
              },
              style: "danger",
