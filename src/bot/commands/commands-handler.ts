...
export function configureCommandsHandler(app: App): void {
  ...
  app.command(/(.*)/, async ({ command, ack, client, respond }) => {
    ...
    await ack();
    const team =
      (await getTeam(command.channel_id)) ??
      (await createTeam(command.channel_id, "[Ditt Team]"));
    ...
    try {
      if (process.env.NODE_ENV !== "production") {
        ...
        if (event.text.endsWith("remind")) {
          ...
          if (team != null) {
            console.info(
              `Would team have been nagged? ${await hasActiveUnnaggedAsk(
                team.id
              )}`
            );
        ...
        if (event.text.endsWith("debug")) {
          ...
          const previousScoredAsk = previousAsked
            ? scoreAsked(previousAsked)
            : null;
      ...
      console.error(e);
      await say(
        "Oi! Noe gikk galt i botten. :( Dersom det skjer igjen, ta kontakt i #helsesjekk-bot."
      );
    }
  });
}
